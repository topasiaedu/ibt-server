...
export const sendMessage = async (payload: any, workflowLogId: string) => {
  const { message, contact_id, workflow_id, postTime, timePostType, postDate } = payload;
  ...
  const { data: contactData, error: contactError } = await supabase
    .from('contacts')
    .select('*')
    .eq('contact_id', contact_id).single();
  ...
  if (matches) {
    matches.forEach((spintax: string) => {
      ...
      messagePayload.text.body = messagePayload.text.body.replace(spintax, randomOption);
    });
  }

  messagePayload.text.body = messagePayload.text.body.replace(/%name%/g, contact.name);
  ...
  try {
    const response = await sendMessageWhatsApp(messagePayload, selectedPhoneNumber);
    ...
    if (newMessageError) {
      logError(newMessageError as unknown as Error, 'Error saving message to database');
      return;
    }
    ...
    if (updateStatusError) {
      logError(updateStatusError as unknown as Error, 'Error updating workflow log status');
      return;
    }
